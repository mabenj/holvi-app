...
export default class Cryptography {
  ...
  static async encryptFile(filepath: string) {
    ...
    return new Promise<void>((resolve, reject) => {
      output.on("finish", () => {
        void rename(filepath + ".temp", filepath).then(resolve);
      });
  ...
  static async getDecryptedStreamChunk(
    filepath: string,
    offset: number,
    chunkSize = env.STREAM_CHUNK_SIZE_KB,
  ): Promise<{
    stream: Stream;
    size: number;
    totalSize: number;
  ...
  private static getCipher(iv: Buffer) {
    return crypto.createCipheriv(ENCRYPTION_ALGORITHM, env.ENCRYPTION_KEY, iv);
  }
